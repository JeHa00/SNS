...
class UserService:
    ...
    def send_verification_email(
        ...
        """매개변수로 받은 email 주소로 이메일 인증 메일을 보낸다.
        ...
        Raises:
            HTTPException (500 INTERNAL SERVER ERROR): 이메일 전송과정에서 error 발생 시, 등록한 유저를 삭제
        """
        try:
    ...
    def send_password_reset_email(
        ...
        """매개변수로 받은 email 주소로 임시 비밀번호를 발송한다.
        ...
        try:
            ...
            self.update(
                ...
                data_to_be_updated={
                    "password": self.get_password_hash(temporary_password)
                },
            )
    ...
    def get_current_user(
        ...
        user = __class__.get_user(db, {"email": email})
    ...
    def get_user(self, db: Session, **kwargs) -> User:
        ...
        Raises:
            HTTPException(400 BAD REQUEST): 입력받은 정보에 해당되는 유저를 찾을 수 없을 경우 발생
        ...
        if len(kwargs) == 1:
            ...
            if not self.verify_password(kwargs.get("password"), user.password):
                raise HTTPException(
                    status_code=400, detail="해당 정보에 해당되는 유저를 찾을 수 없습니다."
                )
            return user
    ...
    def create(self, db: Session, user_info: schema.UserCreate) -> User:
        ...
        Args:
            db (Session): db session
            user_info (schema.UserCreate): 등록할 user의 가입 정보

        Returns:
            User: 생성된 user 객체
        """
        user_info.password = self.get_password_hash(user_info.password)
        new_user = user_crud.create(db, user_info=user_info)
        return new_user
    ...
    def update(
        ...
        Returns:
            User: 수정된 user 객체
        """
        updated_user = user_crud.update(db, user, data_to_be_updated=data_to_be_updated)
        return updated_user
    ...
    def remove(self, db: Session, user_info: User | int) -> dict:
        ...
        Args:
            db (Session): db session
            user_info (User | int): 삭제될 user 정보

        Returns:
            dict: 작업 성공 메세지
        """
        user_crud.remove(db, user_info=user_info)
