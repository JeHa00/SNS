...
def get_multi_comments(db: Session, skip: int = 0, limit: int = 100, **kwargs) -> List[Comment]:
    """writer_id에 해당되는 writer가 작성하는 comment들, 또는 post_id에 해당되는 post에 작성된 comment들을 조회한다.  
    ...
    """
    if kwargs["writer_id"]:
        return (db.query(Comment).filter(Comment.writer_id == kwargs["writer_id"])
            .offset(skip).limit(limit)
            .all())
    elif kwargs["post_id"]:
        return (db.query(Comment).filter(Comment.post_id == kwargs["post_id"])
            .offset(skip).limit(limit)
            .all())


def create(db: Session, data_to_be_created: CommentCreate, writer_id: int, post_id: int) -> Comment:
...
def update(db: Session, comment_info: Comment | int, data_to_be_updated: CommentUpdate) -> Comment:
...
def remove(db: Session, comment_info: Comment | int):
